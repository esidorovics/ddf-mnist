...
class VAE(nn.Module):
    def __init__(self, img_size, latent_dim, ddf_dim, use_cuda=False):
        """
        ...
        self.ddf_input = nn.Linear(self.hidden_dim, self.ddf_dim)
        self.ddf_output = nn.Linear(self.ddf_dim, self.hidden_dim)
        ...
        products = []
        for i in range(self.ddf_dim):
            for j in range(i+1, self.ddf_dim):
                w1 = w[i]
                w2 = w[j]
                prod = np.dot(w1, w2) + b_input[i]
                products.append(prod)
        shift = np.percentile(products, 80)
        ...
        self.latent_to_ddf = nn.Sequential(
            nn.Linear(self.latent_dim, self.hidden_dim),
            nn.ReLU(),
            self.ddf_input
        )

        self.ddf_to_features = nn.Sequential(
            nn.ReLU(),
            self.ddf_output,
            nn.ReLU(),
    ...
    def decode(self, ddf):
        ...
        features = self.ddf_to_features(ddf)
    ...
    def forward(self, x):
        ...
        ddf = self.latent_to_ddf(latent_sample)
        return self.decode(ddf), (mean, log_var), ddf
